...
export class DockerService {
  ...
  public async getContainerLogs(name: string, options: { follow?: boolean; tail?: number } = {}): Promise<NodeJS.ReadableStream | null> {
    try {
      const container = await this.getContainer(name);
      if (!container) {
        logger.error(`Container ${name} not found`);
  ...
  public async exec(containerName: string, command: string[]): Promise<{ success: boolean; stdout: string; stderr: string }> {
    try {
