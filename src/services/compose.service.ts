...
export class ComposeService {
  ...
  public async up(options: ComposeOptions & { detach?: boolean; build?: boolean } = {}): Promise<boolean> {
    try {
  ...
  public async build(options: ComposeOptions & { noCache?: boolean; pull?: boolean } = {}): Promise<boolean> {
    try {
      const command = this.getComposeCommand(options);
      command.push('build');
  ...
  public async exec(service: string, command: string[], options: ComposeOptions & { interactive?: boolean; tty?: boolean } = {}): Promise<boolean> {
    try {
  ...
  public async loadComposeFile(filePath?: string): Promise<DockerComposeConfig | null> {
    try {
      ...
      const fullPath = path.isAbsolute(composeFilePath) ? composeFilePath : path.join(process.cwd(), composeFilePath);
