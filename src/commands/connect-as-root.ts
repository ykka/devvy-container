...
export async function connectAsRootCommand(options: ConnectAsRootOptions): Promise<void> {
  try {
    ...
    logger.info(`Connecting to container as root via ${connectionMethod.toUpperCase()}...`);
    ...
    if (options.mosh) {
      command = 'mosh';
      args = ['--ssh', `ssh -p ${CONSTANTS.SSH.PORT} -i ${sshKeyPath}`, 'root@localhost'];
      ...
      command = 'ssh';
      args = ['-p', String(CONSTANTS.SSH.PORT), '-i', sshKeyPath, 'root@localhost'];
