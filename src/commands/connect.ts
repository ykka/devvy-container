...
export async function connectCommand(options: ConnectOptions): Promise<void> {
  ...
  try {
    ...
    const sshKeyPath =
      sshConfig.keyPath ||
      path.join(process.cwd(), CONSTANTS.PATHS.SECRETS_DIR, CONSTANTS.SSH.KEY_NAME)
    ...
    if (options.mosh && !(await Shell.commandExists('mosh'))) {
    ...
    if (options.mosh) {
      ...
      args = [
        '--ssh',
        `ssh -p ${sshConfig.port} -i ${sshKeyPath} -o StrictHostKeyChecking=no`,
    ...
    const exitCode = await Shell.execInteractive(command, args)
