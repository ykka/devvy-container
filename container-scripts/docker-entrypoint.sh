...
if [ -d "/home/devvy/.config/nvim" ] && [ -f "/home/devvy/.config/nvim/init.lua" ]; then
  # Check if devvy group exists before attempting Neovim plugin installation
  if getent group devvy >/dev/null 2>&1; then
    ...
    if ! timeout 30s su - devvy -c 'nvim --headless "+Lazy! sync" +qa'; then
      echo "[INIT] WARNING: Neovim plugin installation failed or timed out - continuing anyway"
    else
      echo "[INIT] Neovim plugins installed successfully"
    fi
  else
    echo "[INIT] WARNING: 'devvy' group not found, skipping Neovim plugin installation"
    echo "[INIT] You may need to install plugins manually after connecting"
  fi
fi
